...
let usersWithManySkills = Object.entries(users);
console.log(usersWithManySkills);
let higestSkills = { name: "", skills: [] };
for (let [name, detail] of usersWithManySkills) {
  if (detail.skills.length > higestSkills.skills.length) {
    higestSkills.skills = detail.skills;
    higestSkills.name = name;
  }
}

let userss = Object.entries(users);
console.log(userss);
let mern = [];
...
const signUp = (username, email, password, confirmPassword) => {
  let users = [...usersArray];
  if (password !== confirmPassword) return "Mismatch password";
...
const signIn = (username, email, password) => {
  let users = [...usersArray]
    ...
    if(userIndex == -1){
      return('user does not have an account signUp instead')
    }
    let user = users[userIndex]
    if(user.password !== password){
...
console.log(signIn("Thomas","thomas@thomas.com", '123333'))
// console.log(signIn("mufidat","thomfffas@thomas.com"))

const rateProduct = (productId, userId, rate) => {
  let allProducts = [...products]
    let product = allProducts.find((product)=> product._id == productId)
    if(!product){
 ...
 console.log(rateProduct('hedfcg','zwf8md', 4))
 ...
 const likeProduct = (productId, userId) => {
    let allProducts = [...products]
      let productIndex = allProducts.findIndex((product)=> product._id == productId)
      if(productIndex === -1) {
          return 'Product is not available'
      }
      // let likeIndex = allProducts[productIndex].likes.indexOf(userId)  
